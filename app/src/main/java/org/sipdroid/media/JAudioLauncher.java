...
{  
   ...
   {  log=logger;
      ...
      useDTMF = (dtmf_pt != 0);
      /*** Initialize the zrtpEngine***/

       zrtpEngine = new ZRTPTransformEngine();
       SimpleZrtpCallback zrtpCallback = new SimpleZrtpCallback(Receiver.mContext);
       zrtpEngine.setUserCallback(zrtpCallback);
       ...
       android.util.Log.i("ZRTP RtpStreamSender", "ZRTP is now initialized: " );
      ...
      try
      {
         ...
         {  printLog("new audio sender to "+remote_addr+":"+remote_port,LogLevel.MEDIUM);
            ...
            sender=new RtpStreamSender(true,payload_type,frame_rate,frame_size,socket,remote_addr,remote_port,call_recorder, zrtpEngine);
            sender.setSyncAdj(2);
         ...
         {  printLog("new audio receiver on "+local_port,LogLevel.MEDIUM);
            ...
            receiver=new RtpStreamReceiver(socket,payload_type,call_recorder, zrtpEngine);
         }
      }
...
//    	  socket.close();
  ...
  public boolean sendDTMF(char c){
   ...
   /** Added by oitalis to pass the application context from the user agent**/
