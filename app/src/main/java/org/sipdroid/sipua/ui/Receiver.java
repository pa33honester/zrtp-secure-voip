...
//			        mContext.sendBroadcast(new Intent(Intent.ACTION_CLOSE_SYSTEM_DIALOGS));
		...
		private static final long UPDATE_COOLDOWN = 1000; // 1 second
		@SuppressLint({"NewApi", "SuspiciousIndentation"})
		@TargetApi(26)
		public static void onText(int type,String text,int mInCallResId,long base) {
			if (System.currentTimeMillis() - lastUpdateTime < UPDATE_COOLDOWN) {
				Log.d("Receiver", "Skipping onText for type " + type + " due to cooldown");
				return;
			}
			...
			if (mContext == null) {
				Log.e("Receiver", "onText: mContext is null, cannot proceed with type " + type);
				return;
			}

			if (text == null && mInCallResId == 0 && base == 0) {
				if (type == AUTO_ANSWER_NOTIFICATION) {

					if (mNotificationMgr != null)
						mNotificationMgr.cancel(type);
					Log.d("Receiver", "Canceled AUTO_ANSWER_NOTIFICATION cleanly");
					return;
				}
			}
	        ...
	        //if (type != AUTO_ANSWER_NOTIFICATION)   	updateAutoAnswer();  commented by oitalis to cancel reccursive call
			boolean inRecursion = true;
			if (mSipdroidEngine != null && otype >= REGISTER_NOTIFICATION && otype != REGISTER_NOTIFICATION+mSipdroidEngine.pref && !inRecursion)
		...
		static void updateAutoAnswer() {
			if (System.currentTimeMillis() - lastUpdateTime < UPDATE_COOLDOWN) {
				Log.d("Receiver", "Skipping updateAutoAnswer due to cooldown");
				return;
			}
			lastUpdateTime = System.currentTimeMillis();
			/*
		...
		*/
		static void updateAutoAnswer(int status) {
			if (status != autoAnswerState) {
				Log.d("Receiver", "Updating autoAnswerState from " + autoAnswerState + " to " + status);
				autoAnswerState = status;
				switch (status) {
					case 0:
						Receiver.onText(Receiver.AUTO_ANSWER_NOTIFICATION, mContext.getString(R.string.auto_disabled), R.drawable.auto_answer_disabled, System.currentTimeMillis());
						break;
					case 1:
						Receiver.onText(Receiver.AUTO_ANSWER_NOTIFICATION, mContext.getString(R.string.auto_enabled), R.drawable.auto_answer, System.currentTimeMillis());
						break;
					case -1:
						Receiver.onText(Receiver.AUTO_ANSWER_NOTIFICATION, null, 0, 0); // Cancels notification
						break;
				}
			}
		}
