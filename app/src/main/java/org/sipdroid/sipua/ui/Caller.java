...
public class Caller extends BroadcastReceiver {
	...
	static String getNumber(Context context, Uri contactRef, String column) {
		String number = "";
		String[] projection = { column, Phone.NUMBER };

		try (Cursor phonesCursor = context.getContentResolver().query(
				contactRef,
				projection,
				null,
				null,
			...
			if (phonesCursor != null && phonesCursor.moveToNext()) {
				int columnIdx = phonesCursor.getColumnIndex(column);
				if (columnIdx >= 0) {
					String id = phonesCursor.getString(columnIdx);
					try (Cursor pCur = context.getContentResolver().query(
							Phone.CONTENT_URI,
						...
						while (pCur != null && pCur.moveToNext()) {
							String n = pCur.getString(pCur.getColumnIndexOrThrow(Phone.NUMBER));
							if (TextUtils.isEmpty(n))
								continue;
							if (!number.isEmpty())
								number += "&";
							n = PhoneNumberUtils.stripSeparators(n);
							number += searchReplaceNumber(context, n);
						}
					}
				}
			}
		} catch (Exception e) {
			Log.e("Caller", "Contact query error", e);
		}
		return number;
	}
	...
	public void onReceive(final Context context, Intent intent) {
		...
		if (ContextCompat.checkSelfPermission(context,
			...
			Log.w("Caller", "Missing READ_CONTACTS permission");
			return;
		}
		...
		if (intentAction.equals(Intent.ACTION_NEW_OUTGOING_CALL) && number != null) {
			...
			if (sip_type && !force) {
				String sExPat = PreferenceManager.getDefaultSharedPreferences(context)
						.getString(Settings.PREF_EXCLUDEPAT, Settings.DEFAULT_EXCLUDEPAT);
			...
			if (!sip_type) {
				...
				if (number != null && !intent.getBooleanExtra("android.phone.extra.ALREADY_CALLED", false)) {
					...
					if (!ask && !force && PreferenceManager.getDefaultSharedPreferences(context)
							.getBoolean(Settings.PREF_PAR, Settings.DEFAULT_PAR)) {
						number = getNumber(context,
								Uri.withAppendedPath(ContactsContract.PhoneLookup.CONTENT_FILTER_URI, number),
								PhoneLookup._ID);
						if (number.equals(""))
					...
					if (!ask && Receiver.engine(context).call(number, force))
						setResultData(null);
							...
							&& PreferenceManager.getDefaultSharedPreferences(context).getBoolean(Settings.PREF_CALLTHRU,
									Settings.DEFAULT_CALLTHRU)
							&&
							(callthru_prefix = PreferenceManager.getDefaultSharedPreferences(context)
									.getString(Settings.PREF_CALLTHRU2, Settings.DEFAULT_CALLTHRU2)).length() > 0) {
			...
			// Add Bluetooth permission check if using Bluetooth features:
			if (RtpStreamReceiver.isBluetoothAvailable() &&
					ContextCompat.checkSelfPermission(context,
							Manifest.permission.BLUETOOTH_CONNECT) != PackageManager.PERMISSION_GRANTED) {
				Log.w("Caller", "Missing BLUETOOTH_CONNECT permission");
				return;
			}
		}
	}
